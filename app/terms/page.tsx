import { SiteFooter, SiteHeader } from "../components";

export default function TermsPage() {
  return (
    <main>
      <SiteHeader />
      <article className="legal-page">
        <p className="eyebrow">Terms, cancellations &amp; medical disclaimer</p>
        <h1>Important information about services and payments</h1>
        <h2>Operating entity</h2>
        <p>Apex Vitality is operated by Elite Functional Health, a Professional Nursing Corporation. Professional services are provided by John Bagby, MSN, FNP-C.</p>
        <h2>Educational content only</h2>
        <p>Website content is provided for general educational purposes and is not medical advice, diagnosis, or treatment. Do not delay or disregard professional care because of information on this website.</p>
        <h2>No clinician-patient relationship</h2>
        <p>Using this website, reviewing pricing, paying a fee, or scheduling an appointment does not by itself create a clinician-patient relationship. Such a relationship can arise only through an appropriate clinical intake, evaluation, and acceptance process.</p>
        <h2>No guarantee of treatment</h2>
        <p>Consultation or membership does not guarantee eligibility, a prescription, any particular medication, or a specific result. Clinical decisions depend on individualized evaluation and applicable law.</p>
        <h2>Prices and outside costs</h2>
        <p>All prices displayed on this website are in U.S. dollars. Fees cover only the professional services expressly listed. Medications, laboratory testing, pharmacy charges, imaging, supplements, and outside services are separate unless expressly stated.</p>
        <h2>Appointment cancellations and refunds</h2>
        <p>Appointments canceled or rescheduled at least 24 hours before the scheduled start time are eligible for a full refund or account credit. Cancellations made with less than 24 hours&apos; notice and missed appointments are subject to a $150 USD fee; any remaining prepaid appointment balance will be returned to the original payment method. If Apex Vitality cancels an appointment, the patient may reschedule or receive a full refund. Approved refunds are submitted promptly, but the patient&apos;s bank or card issuer controls when the credit appears.</p>
        <h2>Membership cancellation</h2>
        <p>Memberships may be canceled before the next renewal through the secure patient portal. Cancellation stops future renewals. Fees already paid for the current membership period are not prorated or refunded except when required by law or expressly agreed in writing.</p>
        <h2>Customer service</h2>
        <p>Use the secure Charm patient portal for appointment changes, assigned forms, non-urgent messages, and other care-related needs. Stripe payment receipts and payment details remain available through Stripe. Medical information should not be sent through ordinary email.</p>
        <h2>Emergency care</h2>
        <p>This website and secure messaging are not emergency services. Call 911 or seek immediate in-person care for urgent or life-threatening concerns.</p>
        <p className="legal-updated">Last updated July 19, 2026.</p>
      </article>
      <SiteFooter />
    </main>
  );
}
